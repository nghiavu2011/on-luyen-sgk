"""
Batch OCR Runner — Hoàn thiện quét 100% toàn bộ SGK Lớp 6 & Lớp 11
Sử dụng Vision Model Qwen2.5-VL 7B Offline trên NVIDIA RTX 3080.
Hỗ trợ checkpoint lưu tiến độ thời gian thực, tự resume và bỏ qua trang đã xong.
"""
import os
import sys
import json
...
RUNNER_LOG = BASE_DIR / "pipeline" / "ocr_grades_6_11.log"
...
def get_subject_and_tier(filename):
    ...
    if 'tin-hoc' in fn:
        return 'tin-hoc', 1
    if 'khoa-hoc-tu-nhien' in fn:
        return 'khoa-hoc-tu-nhien', 1
    if 'toan' in fn:
        return 'toan', 1
    if 'ngu-van' in fn:
        return 'ngu-van', 1
    ...
    if 'sinh-hoc' in fn:
        return 'sinh-hoc', 1
    if 'lich-su-va-dia-li' in fn:
        return 'lich-su-dia-li', 1
    if 'mi-thuat' in fn:
        return 'mi-thuat', 2
    if 'lich-su' in fn:
        return 'lich-su', 1
    ...
    if 'tieng-anh' in fn:
        return 'tieng-anh', 1
    if 'giao-duc-cong-dan' in fn:
        return 'giao-duc-cong-dan', 1
...
def discover_target_books():
    """Lấy danh sách toàn bộ sách Lớp 6 và 11 cần quét, sắp xếp theo thứ tự ưu tiên."""
    status = load_status()
    books = []
    
    grade_folders = [
        (6, "PDF SGK Lớp 06 2026-2027", "grade-06"),
        (11, "PDF SGK Lớp 11 2026-2027", "grade-11")
    ]
    
    for g_num, folder_name, grade_id in grade_folders:
    ...
    # Sắp xếp: Tier 1 (Cốt lõi) trước -> Tier 2 (Kỹ năng/Nghệ thuật) -> Tier 3 (Chuyên đề) -> Tier 4 (Ngoại ngữ phụ)
    # Trong cùng tier: Lớp 6 trước, sau đó Lớp 11
    books.sort(key=lambda x: (x["tier"], x["grade_num"], x["subject"], x["slug"]))
    return books

def ocr_single_page(doc, page_num, dpi=110):
    """Render trang PDF và gọi OCR qua Qwen2.5-VL."""
    page = doc[page_num - 1]
...
def run_pipeline():
    books = discover_target_books()
    status = load_status()
    
    log("=" * 75)
    log("🚀 KHỞI ĐỘNG HỆ THỐNG QUÉT 100% TOÀN BỘ CÁC MÔN HỌC SGK LỚP 6 & LỚP 11")
    log(f"🔥 GPU Engine: {MODEL_NAME} trên NVIDIA GeForce RTX 3080")
    log(f"📚 Tổng số đầu sách SGK trong danh mục: {len(books)} cuốn (đã loại trừ SGV)")
    log("=" * 75)
    ...
    for idx, item in enumerate(books, 1):
        ...
        for p in range(1, total_pages + 1):
            ...
            if p in scanned and cache_file.exists() and cache_file.stat().st_size > 30:
                continue
    ...
    log("\n🎉 TOÀN BỘ CÁC MÔN HỌC SGK LỚP 6 VÀ LỚP 11 ĐÃ HOÀN THÀNH 100%!")
